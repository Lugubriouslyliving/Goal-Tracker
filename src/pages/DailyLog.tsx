    ...
    <div className="p-6 max-w-2xl mx-auto">
      ...
      {showForm && (
        <form
          ...
          className="bg-zinc-900 border border-zinc-800/60 rounded-lg p-4 mb-4 space-y-3"
        >
          <input
            type="text"
              ...
              >
                {CATEGORIES.map((c) => (
                  <option key={c} value={c}>
                    {c.charAt(0).toUpperCase() + c.slice(1)} — {XP_PER_CATEGORY[c]} XP/session
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className="flex items-center justify-between pt-1">
            <span className="text-xs text-zinc-500">
              ...
              {form.duration >= 60 && (
                <span className="text-zinc-600 ml-1">(1.5× bonus for 60+ min)</span>
              )}
            </span>
            <button
              type="submit"
              className="bg-emerald-600 hover:bg-emerald-500 text-white text-sm px-4 py-1.5 rounded transition-colors"
            >
              Add Log
            </button>
          </div>
      ...
      {dayLogs.length > 0 && (
        <div className="flex items-center justify-between mb-3">
          ...
          <p className="text-xs text-emerald-600">
            +{dayLogs.reduce((s, l) => s + l.xpEarned, 0)} XP total
          </p>
        </div>
      )}
      ...
      <div className="space-y-2">
        {dayLogs.length === 0 ? (
          ...
          dayLogs.map((log) => (
            <div
              key={log.id}
              className="bg-zinc-900 border border-zinc-800/60 rounded-lg p-4 flex items-start justify-between group"
            >
